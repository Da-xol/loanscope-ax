...
if menu == "기능소개":
    ...
    cases = load_cases()
    st.markdown('<div class="loan-shell loan-intro"><div class="notice">본 서비스는 대출 승인·거절 또는 부정행위를 자동 판정하지 않습니다. 공간정보와 제출자료의 불일치 신호를 제공하는 직원용 심사보조 도구입니다.</div></div>', unsafe_allow_html=True)

    case_name = st.selectbox("연결 심사사례", list(cases.keys()), key="loan_case")
    case = cases[case_name]

    st.markdown(f'''<div class="loan-shell"><div class="loan-case-strip"><div class="loan-case-box"><small>심사 대상</small><b>{case["company_name"]}</b></div><div class="loan-case-box"><small>연결품의번호</small><b>{case.get("linked_approval_no", "XXXXXXXXXXXXXXXX")}</b></div><div class="loan-case-box"><small>연결신청번호</small><b>{case.get("linked_application_no", "XXXXXXXXXXXXXXXX")}</b></div></div></div>''', unsafe_allow_html=True)

    nav_col, main_col = st.columns([0.19, 0.81], gap="large")
    with nav_col:
        st.markdown(f'''<nav class="loan-nav" aria-label="LoanScope AX 단계"><div class="loan-nav-case"><strong>{case["company_name"]}</strong><span>{case["loan_purpose"]}</span></div><a id="loan-nav-1" class="loan-link active" href="#loan-step-1"><span class="n">1</span><span><b>기본정보</b><small>기업·대출·시설</small></span></a><a id="loan-nav-2" class="loan-link" href="#loan-step-2"><span class="n">2</span><span><b>서류확인</b><small>필수 증빙 점검</small></span></a><a id="loan-nav-3" class="loan-link" href="#loan-step-3"><span class="n">3</span><span><b>이미지 비교</b><small>공간영상·현장사진</small></span></a><a id="loan-nav-4" class="loan-link" href="#loan-step-4"><span class="n">4</span><span><b>위험신호</b><small>메타데이터·일치성</small></span></a><a id="loan-nav-5" class="loan-link" href="#loan-step-5"><span class="n">5</span><span><b>심사결과</b><small>등급·방문 권고</small></span></a></nav>''', unsafe_allow_html=True)
    ...
    components.html('''<script>(function(){const doc=window.parent.document;const ids=['loan-step-1','loan-step-2','loan-step-3','loan-step-4','loan-step-5'];function active(id){ids.forEach((sid,i)=>{const link=doc.getElementById('loan-nav-'+(i+1));if(link)link.classList.toggle('active',sid===id);});}function bind(){const sections=ids.map(id=>doc.getElementById(id)).filter(Boolean);if(!sections.length){setTimeout(bind,300);return;}ids.forEach((id,i)=>{const link=doc.getElementById('loan-nav-'+(i+1));if(link&&!link.dataset.bound){link.dataset.bound='1';link.addEventListener('click',e=>{e.preventDefault();const target=doc.getElementById(id);if(target){target.scrollIntoView({behavior:'smooth',block:'start'});active(id);}});}});const observer=new IntersectionObserver(entries=>{const visible=entries.filter(e=>e.isIntersecting).sort((a,b)=>a.boundingClientRect.top-b.boundingClientRect.top);if(visible.length)active(visible[0].target.id);},{root:null,rootMargin:'-140px 0px -58% 0px',threshold:[0,.1,.25]});sections.forEach(s=>observer.observe(s));}bind();})();</script>''', height=0, width=0)
